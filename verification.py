...
class Github_requests:
    ...
    def commit_verification(self, message):
        errors = []
        if not (re.match(r'LEETCODE-102', message) or re.match(r'GENERATOR-102', message) or re.match(r'HEXNUMBER-102', message) or re.match(r'TRIANGLE-102', message) or re.match(r'VERIFICATION-102', message) or re.match(r'ITERATOR-102', message)):
            errors.append('Identificator should be LEETCODE/GENERATOR/HEXNUMBER/TRIANGLE/ITERATOR/VERIFICATION-1021/1022.')
        if not (re.search(r'Added', message) or re.search(r'Refactored', message) or re.search(r'Deleted', message) or re.search(r'Fixed', message)):
    ...
    def error_message(self, pullrequest):
        ...
        print("Request - {}".format(pullrequest['url']))
        print(output)
        # output отправляется в pullrequest на гитхаб
